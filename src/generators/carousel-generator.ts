...
export class CarouselGenerator {
   ...
   */
  async generate(signal: Signal, useViralData: boolean = true, editFeedback?: string): Promise<CarouselGenerationResult> {
    ...
    try {
      ...
      const carouselContent = await this.writer.generateCarousel(signal, this.brand, viralInsights, editFeedback);
